...
export default function BillsScreen() {
  ...
  const handleDownloadReportPdf = async (customFrom?: string, customTo?: string) => {
    ...
    try {
      ...
      const fileName = `Sales_Bills_${statusParam}_${rangeTag}.pdf`;

      let pdfUrl = `${API_BASE_URL}/invoices/report-pdf?business_id=${bId}&status=${statusParam}`;
      ...
      if (downloadResult.status === 200) {
        ...
        await savePdfToAndroidOrShare(downloadResult.uri, fileName, 'Sales Report', 'Sales Bills');
      } else {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      {/* Date Range Picker Modal for Report PDF */}
      <Modal
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 20 + insets.bottom }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <View>
                <Text style={{ fontSize: 16, fontWeight: '700', color: Colors.text }}>Export Sales PDF</Text>
