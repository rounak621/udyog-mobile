...
export default function RentalOverdueScreen() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/rental-orders/overdue/list?business_id=${bId}`);
      setOrders(res.data);
    } catch (err) {
      console.log('Error loading overdue orders:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const getDaysOverdue = (endDateStr: string) => {
    ...
    return `${Math.max(1, diffDays)} days overdue`;
  };
        ...
        {filteredOrders.length === 0 ? (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>
              {searchQuery ? 'No matching overdue orders' : 'No overdue orders found'}
            </Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  statusBadge: { borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, backgroundColor: '#FEF2F2' },
  statusBadgeText: { fontSize: 10, fontWeight: '600', color: Colors.danger },
